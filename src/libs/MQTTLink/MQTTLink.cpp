...
{
    MQTTLink *link = static_cast<MQTTLink *>(arg);
    if (link)
    {
        if (status == MQTT_CONNECT_ACCEPTED)
        {
            printf("[mqtt_connection_cb] Connexion établie avec succès !\n");
            link->mqtt_is_connected = true;
        }
        else
        {
            printf("[mqtt_connection_cb] Déconnexion ou erreur : %d\n", status);
            link->mqtt_is_connected = false;
        }
    }
    else
    {
        printf("[mqtt_connection_cb] arg MQTTLink * nullptr!\n");
    }
}
...
static void mqtt_pub_request_cb(void *arg, err_t result)
{
    ...
    if (link)
    {
        ...
        link->publish_in_progress = false;
    }
    else
    {
        printf("[mqtt_pub_request_cb] arg MQTTLink * nullptr!\n");
    }
}
...
{
    printf("[mqtt_incoming_publish_cb] Arrivee de donnees taille=%u)\n", len);
    ((IMqttDispatcher *)arg)->dispatchData(data, len, flags);
}
...
static void mqtt_incoming_publish_cb(void *arg, const char *topic, u32_t tot_len)
{
    ...
    ((IMqttDispatcher *)arg)->dispatchPublish(topic, tot_len);
}
...
static void mqtt_sub_request_cb(void *arg, err_t result)
{
    ...
    if (link)
    {
        if (result == ERR_OK)
        {
            ...
            link->mqtt_sub_ready = true; // On lève le drapeau!
        }
        else
        {
    ...
    {
        printf("[mqtt_sub_request_cb] arg MQTTLink * nullptr!\n");
    }
}
...
MqttStatus MQTTLink::mqtt_connectTimeOut(ip_addr_t *broker_ip, int timeout_ms)
{
    ...
    if (static_client != NULL)
    {
        ...
        err_t err = mqtt_client_connect(static_client, broker_ip, 1883, mqtt_connection_cb, this, &ci);
        cyw43_arch_lwip_end();
        ...
        else
        {
            ...
            if (mqtt_is_connected == true)
            {
                ...
                // Configurer les callbacks de réception (Indispensable avant de s'abonner).
                mqtt_set_inpub_callback(static_client, mqtt_incoming_publish_cb, mqtt_incoming_data_cb, dispatcher);
            }
            else
            {
...
MqttStatus MQTTLink::mqtt_publishTimeOut(const char *topic, const void *msg_buffer, int timeout_ms)
{
    ...
    err_t status = mqtt_publish(static_client, topic, msg_buffer, strlen((const char *)msg_buffer), 1, 0, mqtt_pub_request_cb, this);
...
MqttStatus MQTTLink::mqtt_subscribeTimeOut(const char *topic, int timeout_ms)
{
    ...
    cyw43_arch_lwip_begin();
    err_t err = mqtt_sub_unsub(static_client, topic, 1, mqtt_sub_request_cb, this, 1);
    cyw43_arch_lwip_end();
    ...
    else
    {
        ...
        {
            printf("[mqtt_subscribeTimeOut] Pas de confimation de l'abonnement à %s aprés %dms !\n",
                   topic,
                   timeout_ms);
            returnValue = MqttStatus::TIMEOUT;
        }
